...
for k = 1:length(SNR_lin)
    ...
    for t = 1:n
        ...
        C_temp_simo(t) = log2(1 + norm(h)^2 * snr); %  % capacity formula of SIMO fixed channel
    end
...
% Display results 
disp(table(SNR_dB.', SNR_lin.', C_ergodic_MIMO.',C_ergodic_SIMO.','VariableNames', {'SNR_dB','SNR_linear','Capacity_fast_fading_MIMO','Capacity_fast_fading_SIMO'}));
disp(table(lambda.',VariableNames={'Eigen values of H*H'}));
figure(4);
plot(SNR_dB, C_ergodic_MIMO, '-o','LineWidth',1.5,'MarkerSize',6, Color=[1 0 0]);
hold on;
plot(SNR_dB, C_ergodic_SIMO, '-o','LineWidth',1.5,'MarkerSize',6, color=[0 0 1]);
hold off;
grid on;
xlabel('SNR (dB)');
ylabel('Ergodic Capacity (bits/s/Hz)');
title('Fast-Fading MIMO vs SIMO Channel Capacity');
legend('Fast-fading MIMO channel capacity','Fast-fading SIMO channel capacity','Location','best');
